...
/** get indices of infeasible points used in branching decisions */
static
SCIP_RETCODE getBranchingIndices(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_CONSHDLR*        conshdlr,           /**< constraint handler for branching data */
   int*                  branchingidx,       /**< allocated array to store branching indices */
   int*                  nbranchingidx,      /**< pointer to store number of branching indices */
   int                   maxnidx             /**< maximum number of possible branching indices */
   )
{
   SCIP_Bool* usedidx;
   SCIP_CONS** conss;
   ...
   assert( branchingidx != NULL );
   assert( nbranchingidx != NULL );
   assert ( maxnidx > 0 );

   SCIP_CALL( SCIPallocBufferArray(scip, &usedidx, maxnidx) );
   ...
   for( c = 0; c < nconss; ++c )
   {
      ...
      {
         usedidx[id1] = TRUE;
         branchingidx[*nbranchingidx] = id1;
         *nbranchingidx += 1;
         printf("%d ", id1);
      }
      if ( ! usedidx[id2] )
      {
         usedidx[id2] = TRUE;
         branchingidx[*nbranchingidx] = id2;
         *nbranchingidx += 1;
         printf("%d ", id2);
      }
   }

   SCIPfreeBufferArray(scip, &usedidx);
...
SCIP_RETCODE addBranchingDecisionConss(
   ...
   SCIP_VAR**            vars,               /**< variable array of the subscip instance */
   SCIP_CONSHDLR*        conshdlr            /**< constraint handler for branching data */
   )
{
   SCIP_CONS** conss;
   ...
   )
{
   ...
   for( v = 0; v < norigvars; ++v )
   {
      ...
      if( SCIPvarGetUbLocal(origvars[v]) < 0.5 )
      {
         ...
         /* collect the constraints/items the variable belongs to */
...
SCIP_RETCODE createPricingModel(
   ...
   int*                  branchingidx,       /**< indices of infeasible points used for branching */
   int                   nbranchingidx       /**< number of indices used for branching */
   )
{
   ...
   if ( nbranchingidx > 0 )
   {
      SCIP_CALL( SCIPallocBlockMemoryArray(subscip, &pricerdata->negselectvars, nbranchingidx) );
      for (i = 0; i < nbranchingidx; ++i)
      {
         (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "negselect_%d", branchingidx[i]);
   ...
   {
      printf("new vars: ");
      SCIP_CALL( SCIPallocBlockMemoryArray(subscip, &pricerdata->negconss, nbranchingidx) );
      SCIP_CALL( SCIPallocBufferArray(subscip, &tmpvars, 2) );
      for (i = 0; i < nbranchingidx; ++i)
      {
         (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "negatecons_%d", branchingidx[i]);

         tmpvars[0] = pricerdata->selectvars[branchingidx[i]];
         tmpvars[1] = pricerdata->negselectvars[i];
         printf("%d ", branchingidx[i]);
   ...
   if ( nbranchingidx > 0 )
   {
      SCIP_CALL( SCIPallocBlockMemoryArray(subscip, &pricerdata->negsepaconss, nbranchingidx) );
      tmpvals[dimension] = -1.0;
      for (i = 0; i < nbranchingidx; ++i)
      {
         (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "negsepacons_%d", branchingidx[i]);

         for (j = 0; j < dimension; ++j)
            tmpvals[j] = Ypoints[branchingidx[i]][j];
   ...
   )
{
   ...
   if ( nbranchingidx > 0 )
   {
      ...
      SCIPfreeBlockMemoryArrayNull(subscip, &pricerdata->negselectvars, nbranchingidx);
   }
   ...
   if ( nbranchingidx > 0 )
   {
      ...
      SCIPfreeBlockMemoryArrayNull(subscip, &pricerdata->negsepaconss, nbranchingidx);
      for (i = 0; i < nbranchingidx; ++i)
      {
         SCIP_CALL( SCIPreleaseCons(subscip, &pricerdata->negconss[i]) );
      }
      SCIPfreeBlockMemoryArrayNull(subscip, &pricerdata->negconss, nbranchingidx);
   }
...
{  /*lint --e{715}*/
   ...
   if ( ! atrootnode )
   {
      SCIP_CALL ( SCIPallocBufferArray(scip, &branchingidx, pricerdata->nY) );
      SCIP_CALL( getBranchingIndices(scip, pricerdata->conshdlr, branchingidx, &nbranchingidx, pricerdata->nY) );
      printf("nbranching: %d\n", nbranchingidx);
   }

   SCIP_CALL( createPricingModel(scip, subscip, pricerdata, branchingidx, nbranchingidx) );
   ...
   SCIP_CALL( freePricingModel(scip, subscip, pricerdata, nbranchingidx) );

   if ( ! atrootnode )
   {
      SCIPfreeBufferArray(scip, &branchingidx);
   }
...
static
SCIP_DECL_PRICERFARKAS(pricerFarkasPattern)
{  /*lint --e{715}*/
   SCIPerrorMessage("method of pattern variable pricer not implemented yet\n");
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludePricerBasic(scip, &pricer, PRICER_NAME, PRICER_DESC, PRICER_PRIORITY, PRICER_DELAY,
         /* pricerRedcostPattern, pricerFarkasPattern, pricerdata) ); */
